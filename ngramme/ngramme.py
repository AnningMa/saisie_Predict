...
class NGramPredictor:
    ...
    def train_from_sentences(self, sentences: List[str]):
        ...
        print(f"train {self.n}-gramme model...")
    ...
    def _clean_and_tokenize(self, sentence: str) -> List[str]:
        ...
        # Supprimer les balises d'anonymat (comme <NAME_123>)
        sentence = re.sub(r"<[A-Z]{3}_\d+>", "", sentence)
        # Supprimer tous les chiffres
        sentence = re.sub(r"\d+", "", sentence)
        # Convertir en minuscules
        sentence = sentence.lower()
        # Supprimer les tirets non situés entre deux lettres
        sentence = re.sub(r"(?<![a-zA-Z])-|-(?![a-zA-Z])", "", sentence)
        # Remplacer la ponctuation (sauf tirets) par des espaces
    ...
    def predict_next_words(self, context_words: List[str], top_k: int = 5) -> List[Tuple[str, float]]:
        ...
        if context in self.ngram_counts:
            ...
            predictions.sort(key=lambda x: x[1], reverse=True) #排序
            return predictions[:top_k]
        # else:      
        #     #si contexte n'exist pas
        #     total_count = sum(self.unigram_counts.values())
        #     most_common = sorted(self.unigram_counts.items(), key=lambda x: x[1], reverse=True)[:top_k]
        #     return [(word, count / total_count) for word, count in most_common]
